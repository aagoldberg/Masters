...
fit_val <- unname(m_bty$coefficients)[1] + evals$bty_avg * unname(m_bty$coefficients)[2] 
residuals <- data.frame(cbind(fit_val, residuals))
names(residuals) <- c("avg_bty", "residuals")
ggplot(residuals, aes(y=abs(residuals), x=fit_val))+geom_point()+geom_hline(yintercept = 0)
...
residuals <- resid(m_bty_gen)
evals$genderadj <- ifelse(evals$gender=="male",1,0)
fit_val <- unname(m_bty_gen$coefficients)[1] + evals$bty_avg * unname(m_bty_gen$coefficients)[2] + evals$genderadj * unname(m_bty_gen$coefficients)[3]
resid <- data.frame(cbind(evals$bty_avg, evals$gender, residuals))
names(resid) <- c("avg_bty", "gender", "residuals")
ggplot(resid, aes(y=abs(residuals), x=fit_val))+geom_point()+geom_hline(yintercept = 0)
ggplot(resid, aes(y=residuals, x=avg_bty))+geom_point()+geom_hline(yintercept = 0)
ggplot(data = resid, aes(y=residuals, x=evals$gender))+geom_boxplot() + geom_point()
```
...
```{r}
ggplot(m_bty_gen, aes(y=abs(residuals), x=avg_bty))+geom_point()+geom_hline(yintercept = 0)
```
