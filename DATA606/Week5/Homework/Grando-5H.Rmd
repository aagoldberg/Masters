...
$$s\quad =\quad ME\quad *\quad \sqrt { n } \quad /\quad Z\quad =\quad 6\quad *\quad \sqrt { 25 } \quad /\quad 1.65\quad =\quad 18.185$$
...
$${ H }_{ O }:\quad { \mu  }_{ less than HS }\quad =\quad { \mu  }_{ HS }\quad =\quad { \mu  }_{ Jr Coll }\quad =\quad { \mu  }_{ Bachelors }\quad=\quad { \mu  }_{ Graduate }\quad\\ { H }_{ A }:\quad { At\ least\ onemean\ is\ different  }\quad$$
...
degree    |    4  | 2,006.2  |   229.13  |    2.19   | 0.0682

residuals | 1,167 | 267,382  |   2364.9  |
...
ggplot(data.frame(x = c(lb, ub)), aes(x = x)) + stat_function(fun = df, args = list(df1=4, df2=1167)) + stat_function(fun = df, args = list(df1=4, df2=1167), xlim = c(f1, ub),geom = "area", alpha=0.5) + geom_vline(xintercept=pick_line1, color = "black", alpha = 0.75)+geom_text(aes(x=pick_line1, y=0.25, label=sprintf("T = %s\n", pick_line1)), color="black", angle=90) + labs(x="F Value")
```
