...
The distributions appear to be slightly to moderately left-skeweed; however, the sample sizes are much larger than 30 so the conditions for inference appear to have been met.
...
There is not sufficient evidence to reject the null Hypothesis.  Therefore, we fail to reject the hypothesis that there is a difference between the average weight gained between younger and mature mothers.
...
There is not sufficient evidence to reject the null hypothesis.  Therefore, we fail to reject the hypothesis that there is a difference of age between the fathers of premie and full term children.

```{r}
inference(y = nc$fage, x = nc$premie, est = "mean", type = "ci", null = 0, alternative = "twosided", method = "theoretical", conflevel = 0.95)
```
