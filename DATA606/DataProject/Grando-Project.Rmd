...
#Work on this one... filter out the smaller countries?  Send them to an "other" category?
(Country_count <- table(filtered_projects_lm$Country))
...
```{r}
ST_lm <- lm(SubmittalTime ~ Isconfidential + Country + RatingSystemFamily + OwnerTypesAdj + log(GrossSqFoot), data = filtered_projects_lm)
summary(ST_lm)

#Backward Radj - First step remove Isconfidential
ST_lm_b <- lm(SubmittalTime ~ Country + RatingSystemFamily + OwnerTypesAdj + log(GrossSqFoot), data = filtered_projects_lm)
summary(ST_lm_b)
...
# summary(ST_lm_f)
# 
# ST_lm_f <- lm(SubmittalTime ~ RatingSystemFamily + OwnerTypesAdj + log(GrossSqFoot) + Country, data = filtered_projects_lm)
# summary(ST_lm_f)
...
ggplot(ST_lm_b, aes(y=resid(ST_lm_b), x=filtered_projects_lm$Country))+geom_boxplot() + labs(x="Country Code", y="Residuals") + ggtitle("Multi-linear Regression Model Residuals By Country") + theme(plot.title = element_text(hjust = 0.5)) + coord_flip()
```
