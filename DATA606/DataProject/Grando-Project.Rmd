...
# Is this necessary for ANOVA? However, since the entire current population is being used, the standard deviation, rather than the standard error, will be used to determine all normalized data due to the fact that the population paratmeters are being used instead of sample parameters.
...
#pooled sd
sd <- sqrt(summary(RSF_aov)[[1]][[3]][[2]])
se <- sqrt(sd^2 / BDC_V3_n + sd^2 / OM_V3_n)
...
#pooled sd
sd <- sqrt(summary(RSF_aov)[[1]][[3]][[2]])
se <- sqrt(sd^2 / BDC_V3_n + sd^2 / IDC_V3_n)
...
#pooled sd
sd <- sqrt(summary(RSF_aov)[[1]][[3]][[2]])
se <- sqrt(sd^2 / OM_V3_n + sd^2 / IDC_V3_n)
...
As can be seen from the pariwise t-tests, it has been determined that we are 95% confident that the mean submittal time between groups is different between all groups (BD+C, ID+C, and OM).

Now, there are not many numerical values that can be used as predictors of how long it will take a project to certify.  Note, attributes such as the number of points achieved and certification level are only known after a project has certified; therefore, have limited value.  One possible numerical indicator of submittal time that can be used in the gross floor area of the project.  We will now use this predictor to determine a least squares linear regerssion line to predict the response variable of submittal time.  Since we have already determined that each Rating System Family has significantly different submittal times, we will perform separate linear regression analsyses for BD+C, ID+C, and OM.

####Generate Linear Regression Models and Graphs
```{r}
#BDC
gf_lm_BDC_V3 <- lm(SubmittalTime ~ GrossSqFoot, data = completed_projects_BDC_V3)
summary(gf_lm_BDC_V3)

ggplot(completed_projects_BDC_V3, aes(y=SubmittalTime, x=GrossSqFoot)) + geom_point()+ geom_smooth(method = lm, fullrange = TRUE) + labs(x="Gross Floor Area", y="Days Between Registration and Certification") + ggtitle("BD+C Gross Floor Area Against Submittal Time") + theme(plot.title = element_text(hjust = 0.5))

#IDC
gf_lm_IDC_V3 <- lm(SubmittalTime ~ GrossSqFoot, data = completed_projects_IDC_V3)
summary(gf_lm_IDC_V3)
ggplot(completed_projects_IDC_V3, aes(y=SubmittalTime, x=GrossSqFoot)) + geom_point()+ geom_smooth(method = lm, fullrange = TRUE) + labs(x="Gross Floor Area", y="Days Between Registration and Certification") + ggtitle("ID+C Gross Floor Area Against Submittal Time") + theme(plot.title = element_text(hjust = 0.5))

#OM
gf_lm_OM_V3 <- lm(SubmittalTime ~ GrossSqFoot, data = completed_projects_OM_V3)
summary(gf_lm_OM_V3)
ggplot(completed_projects_OM_V3, aes(y=SubmittalTime, x=GrossSqFoot)) + geom_point()+ geom_smooth(method = lm, fullrange = TRUE) + labs(x="Gross Floor Area", y="Days Between Registration and Certification") + ggtitle("OM Gross Floor Area Against Submittal Time") + theme(plot.title = element_text(hjust = 0.5))

```

As we can see from the R-square values, these models do not appear to be the best predictors of submitall time but do produce statistically significant prediction slopes.  While these results do not appear to be of much use at the moment, we will continue the analysis and validate the models by checking the following asusmptions:

1. the residuals of the model are nearly normal

```{r}
#BDC
ggplot(data = gf_lm_BDC_V3, aes(x=resid(gf_lm_BDC_V3))) + geom_histogram(binwidth = 100, position = "identity", aes(y=..density..)) + stat_function(fun=dnorm,color="black",args=list(mean=mean(gf_lm_BDC_V3$residuals),sd(gf_lm_BDC_V3$residuals))) + labs(x="Residuals") + ggtitle("BD+C Gross Floor Area Residuals") + theme(plot.title = element_text(hjust = 0.5))
qqnorm(gf_lm_BDC_V3$residuals)
qqline(gf_lm_BDC_V3$residuals)

#IDC
ggplot(data = gf_lm_IDC_V3, aes(x=resid(gf_lm_IDC_V3))) + geom_histogram(binwidth = 100, position = "identity", aes(y=..density..)) + stat_function(fun=dnorm,color="black",args=list(mean=mean(gf_lm_IDC_V3$residuals),sd(gf_lm_IDC_V3$residuals))) + labs(x="Residuals") + ggtitle("ID+C Gross Floor Area Residuals") + theme(plot.title = element_text(hjust = 0.5))
qqnorm(gf_lm_IDC_V3$residuals)
qqline(gf_lm_IDC_V3$residuals)

#OM
ggplot(data = gf_lm_OM_V3, aes(x=resid(gf_lm_OM_V3))) + geom_histogram(binwidth = 100, position = "identity", aes(y=..density..)) + stat_function(fun=dnorm,color="black",args=list(mean=mean(gf_lm_OM_V3$residuals),sd(gf_lm_OM_V3$residuals))) + labs(x="Residuals") + ggtitle("OM Gross Floor Area Residuals") + theme(plot.title = element_text(hjust = 0.5))
...
```{r}
#BDC
ggplot(gf_lm_BDC_V3, aes(y=abs(resid(gf_lm_BDC_V3)), x=gf_lm_BDC_V3$fitted.values))+geom_point()+geom_hline(yintercept = 0) + labs(x="Fitted Values", y="Absolute Values of residuals") + ggtitle("BD+C Gross Floor Area Fitted Values Against Residuals") + theme(plot.title = element_text(hjust = 0.5))
#IDC
ggplot(gf_lm_IDC_V3, aes(y=abs(resid(gf_lm_IDC_V3)), x=gf_lm_IDC_V3$fitted.values))+geom_point()+geom_hline(yintercept = 0) + labs(x="Fitted Values", y="Absolute Values of residuals") + ggtitle("ID+C Gross Floor Area Fitted Values Against Residuals") + theme(plot.title = element_text(hjust = 0.5))
#OM
ggplot(gf_lm_OM_V3, aes(y=abs(resid(gf_lm_OM_V3)), x=gf_lm_OM_V3$fitted.values))+geom_point()+geom_hline(yintercept = 0) + labs(x="Fitted Values", y="Absolute Values of residuals") + ggtitle("OM Gross Floor Area Fitted Values Against Residuals") + theme(plot.title = element_text(hjust = 0.5))
```
...
```{r}
#BDC
ggplot(gf_lm_BDC_V3, aes(y=resid(gf_lm_BDC_V3), x=completed_projects_BDC_V3$RegistrationDate))+geom_point()+geom_hline(yintercept = 0) + labs(x="Registration Date", y="Residuals") + ggtitle("BD+C Registration Date Against Residuals") + theme(plot.title = element_text(hjust = 0.5))
#IDC
ggplot(gf_lm_IDC_V3, aes(y=resid(gf_lm_IDC_V3), x=completed_projects_IDC_V3$RegistrationDate))+geom_point()+geom_hline(yintercept = 0) + labs(x="Registration Date", y="Residuals") + ggtitle("ID+C Registration Date Against Residuals") + theme(plot.title = element_text(hjust = 0.5))
#OM
ggplot(gf_lm_OM_V3, aes(y=resid(gf_lm_OM_V3), x=completed_projects_OM_V3$RegistrationDate))+geom_point()+geom_hline(yintercept = 0) + labs(x="Registration Date", y="Residuals") + ggtitle("OM Registration Date Against Residuals") + theme(plot.title = element_text(hjust = 0.5))
```
...
```{r}
#BDC
ggplot(gf_lm_BDC_V3, aes(y=resid(gf_lm_BDC_V3), x=completed_projects_BDC_V3$GrossSqFoot))+geom_point()+geom_hline(yintercept = 0) + labs(x="Gross Floor Area", y="Residuals") + ggtitle("BD+C Gross Floor Area Against Residuals") + theme(plot.title = element_text(hjust = 0.5))
#IDC
ggplot(gf_lm_IDC_V3, aes(y=resid(gf_lm_IDC_V3), x=completed_projects_IDC_V3$GrossSqFoot))+geom_point()+geom_hline(yintercept = 0) + labs(x="Gross Floor Area", y="Residuals") + ggtitle("ID+C Gross Floor Area Against Residuals") + theme(plot.title = element_text(hjust = 0.5))
#OM
ggplot(gf_lm_OM_V3, aes(y=resid(gf_lm_OM_V3), x=completed_projects_OM_V3$GrossSqFoot))+geom_point()+geom_hline(yintercept = 0) + labs(x="Gross Floor Area", y="Residuals") + ggtitle("OM Gross Floor Area Against Residuals") + theme(plot.title = element_text(hjust = 0.5))
```

The histogram shows that the data do not exactly follow a normal distribution.  From the normal probability plot it appears the data is narrower than a typical normal distribution (short tails) and has a right ske for the BD+C and ID+C data sets.  The OM data set appear to have a right skew and it is unclear if there are short tails.  Each group has some extreme gross floor area values which are points of high leverage and may be significantly affecting the model.  The residuals appear to be independent from the registration date vs residuals and gross floor area vs residuals graphs.  Note that there is apparent decrease of the maximum possible value in the registration date vs residuals graph.  This is due to the fact that older projects have a larger window to certify since they have been open longer, thus they can have a larger maximum possible residual.  There are some issues with these models, mainly the residual plots being skewed and smaller than expected spas, and a low r-squared value.  Even if all the assumptions were completely validated, the model would only predict `r round(summary(gf_lm_BDC_V3)[[9]] * 100, digits = 4)`% of the variability for the BD+C data set, `r round(summary(gf_lm_IDC_V3)[[9]] * 100, digits = 4)`% of the variability for the ID+C data set, and `r round(summary(gf_lm_OM_V3)[[9]] * 100, digits = 4)`% of the variability for the OM data set, which is much too small to be considered useful.

As previously noted, the gross floor is not a great predictor variable of the time it takes for a project to certify; however, we can perform a multi-linear regression analysis, which uses more than one predictor variable, to estimate the submittal times.  In order to do this, we must filter the data to ensure there are sufficient values in each predictor variable we want to use and then perform an analysis.  For this study, we will start with the following variables since they are known at the time of registration: confidential status (Isconfidential), Country, Rating System Family, owner types (OwnerTypesAdj - Note - the number of categories has been significantly reduced form meaningfullness), and gross floor area (GrossSqFoot).
...
ST_lm_b <- lm(SubmittalTime ~ Country + RatingSystemFamily + OwnerTypesAdj + GrossSqFoot, data = filtered_projects_lm)
summary(ST_lm_b)
...
# summary(ST_lm_f)
# 
# ST_lm_f <- lm(SubmittalTime ~ Country, data = filtered_projects_lm)
# summary(ST_lm_f)
# 
...
# summary(ST_lm_f)
# 
# ST_lm_f <- lm(SubmittalTime ~ GrossSqFoot, data = filtered_projects_lm)
# summary(ST_lm_f)
# 
# 
...
# summary(ST_lm_f)
# 
# ST_lm_f <- lm(SubmittalTime ~ RatingSystemFamily + Country, data = filtered_projects_lm)
# summary(ST_lm_f)
# 
# ST_lm_f <- lm(SubmittalTime ~ RatingSystemFamily + OwnerTypesAdj, data = filtered_projects_lm)
# summary(ST_lm_f)
# 
# ST_lm_f <- lm(SubmittalTime ~ RatingSystemFamily + GrossSqFoot, data = filtered_projects_lm)
# summary(ST_lm_f)
# 
...
# summary(ST_lm_f)
# 
# ST_lm_f <- lm(SubmittalTime ~ RatingSystemFamily + OwnerTypesAdj + Country, data = filtered_projects_lm)
# summary(ST_lm_f)
# 
# ST_lm_f <- lm(SubmittalTime ~ RatingSystemFamily + OwnerTypesAdj + GrossSqFoot, data = filtered_projects_lm)
# summary(ST_lm_f)
# 
# #Forward pick RatingSystemFamily and OwnerTypesAdj and Country
# ST_lm_f <- lm(SubmittalTime ~ RatingSystemFamily + OwnerTypesAdj + Country + Isconfidential, data = filtered_projects_lm)
# summary(ST_lm_f)
# 
# ST_lm_f <- lm(SubmittalTime ~ RatingSystemFamily + OwnerTypesAdj + Country + GrossSqFoot, data = filtered_projects_lm)
# summary(ST_lm_f)

#Forward pick RatingSystemFamily and OwnerTypesAdj and Country and GrossSqFoot
# ST_lm_f <- lm(SubmittalTime ~ RatingSystemFamily + OwnerTypesAdj + Country + GrossSqFoot, data = filtered_projects_lm)
# summary(ST_lm_f)
...
From this analysis, we can see Country, Rating System Family, owner types, and gross floor area are good predictors for the model.  Additionally, we see that the model predicts `r round(summary(ST_lm)[[9]] * 100, digits = 4)`% of the variability for the overall data set, which is much better than the previous model.  However,  in order to validate the model, the following assumptions must be checked:
...
```{r}
ggplot(ST_lm_b, aes(y=resid(ST_lm_b), x=filtered_projects_lm$Country))+geom_boxplot() + geom_point() + labs(x="Country Code", y="Residuals") + ggtitle("Multi-linear Regression Model Residuals By Country") + theme(plot.title = element_text(hjust = 0.5))
ggplot(ST_lm_b, aes(y=resid(ST_lm_b), x=filtered_projects_lm$RatingSystemFamily))+geom_boxplot() + geom_point() + labs(x="Rating System Family", y="Residuals") + ggtitle("Multi-linear Regression Model Residuals By Rating System Family") + theme(plot.title = element_text(hjust = 0.5))
ggplot(ST_lm_b, aes(y=resid(ST_lm_b), x=filtered_projects_lm$OwnerTypesAdj))+geom_boxplot() + geom_point() + labs(x="Owner Types", y="Residuals") + ggtitle("Multi-linear Regression Model Residuals By Owner Type") + theme(plot.title = element_text(hjust = 0.5))
ggplot(ST_lm_b, aes(y=resid(ST_lm_b), x=filtered_projects_lm$GrossSqFoot))+geom_point()+geom_hline(yintercept = 0) + labs(x="Gross Floor Area", y="Residuals") + ggtitle("Multi-linear Regression Model Residuals By Gross Floor Area") + theme(plot.title = element_text(hjust = 0.5))
```

As with the previous linear model, the residual histogram plot shows that the data distribution is more narrow than a normal distribution and has a right skew.  Additionally, since we have kept the gross square foot predictor, there are still some leverage points for this value.  However, the remaining data show that each predictor apepars to be linearly related to the outcome and appear to be independent based on the registration date graph.


#Is the gross floor area plot sloping down?

### Part 5 - Conclusion: 

Note the variance issue in the ANOVA test assumptions.  The issues in the single predictor linear regression and multi-linear
...
This appendix contains additional analysis work that will be completed after this course.

```{r}
#Make a function that is a predictor of the linear regression model and apply it to all open projects.  Then you will have a summary of all the expected dates they will certify.

#Do the same for the glm and you will have a predicted percentage of certification to multiply it by as well.
...
ST_glm <- glm(IsCertifiedBol ~ RatingSystemFamily + OwnerTypesAdj + Country + GrossSqFoot + TimeOpen, data = filtered_projects_glm)
summary(ST_glm)

#Coordinte with Ops on the LOE for each project and you will have a model for future man-hours needed.
```
