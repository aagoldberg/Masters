...
#All significant predictors but not a good r-square fit or pracitcally sginificant?  Make case for multi-linear regression.
...
```{r}
ggplot(data = ST_lm_b, aes(x=resid(ST_lm_b))) + geom_histogram(binwidth = 100, position = "identity", aes(y=..density..)) + stat_function(fun=dnorm,color="black",args=list(mean=mean(ST_lm_b$residuals),sd(ST_lm_b$residuals)))
...
```{r}
ggplot(ST_lm_b, aes(y=resid(ST_lm_b), x=filtered_projects_lm$RegistrationDate))+geom_point()+geom_hline(yintercept = 0)
```
...
```{r}
ggplot(ST_lm_b, aes(y=resid(ST_lm_b), x=filtered_projects_lm$Country))+geom_boxplot() + geom_point()
ggplot(ST_lm_b, aes(y=resid(ST_lm_b), x=filtered_projects_lm$RatingSystemFamily))+geom_boxplot() + geom_point()
ggplot(ST_lm_b, aes(y=resid(ST_lm_b), x=filtered_projects_lm$OwnerTypesAdj))+geom_boxplot() + geom_point()
ggplot(ST_lm_b, aes(y=resid(ST_lm_b), x=filtered_projects_lm$GrossSqFoot))+geom_point()+geom_hline(yintercept = 0)
```
