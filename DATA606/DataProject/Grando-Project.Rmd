...
As we can see from the R-square values (BD+C - `r round(summary(gf_lm_BDC_V3)[[9]],digits=2)`, ID+C - `r round(summary(gf_lm_IDC_V3)[[9]],digits=2)`, OM - `r round(summary(gf_lm_OM_V3)[[9]],digits=2)`), these models do not appear to be the best predictors of submitall time but do produce statistically significant prediction slopes.  While these results do not appear to be of much use at the moment, we will continue the analysis and validate the models by checking the following asusmptions:
...
####Leverage points
In the analysis, it was determined that older projects may be singificant leverage points.  Since they have been open for so much longer than the newer projects, they have an increased ability to deviate from the model.  Conversely, very new projects may also have singificant leverage points since we would only end up seeing projects that have certified quickly and may not represent the entire group of submittals for that time period.  In a separate analysis () I have created a similar analysis which excludes the oldest and youngest completed projects to see the overall impact.
