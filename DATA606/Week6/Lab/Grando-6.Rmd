...
Yes, the number of successes (atheist) are `r nrow(us12_atheist)` and failures (non-athiest) are `r nrow(us12)-nrow(us12_atheist)`.
...
As with the US sample, it appears we can assume the samples were independent.  Additionally, the number of successes (atheist) are `r nrow(g12_atheist)` and failures (non-athiest) are `r nrow(g12)-nrow(g12_atheist)`.
...
As with the US sample, it appears we can assume the samples were independent for those respondents from Peru.  Additionally, the number of successes (atheist) are `r nrow(p12_atheist)` and failures (non-athiest) are `r nrow(p12)-nrow(p12_atheist)`.
...
The number of successes (atheist) are `r nrow(au12_atheist)` and failures (non-athiest) are `r nrow(au12)-nrow(au12_atheist)` for the australian respondents.  This meets the minimum requirements to proceed.
...
The number of successes (atheist) are `r nrow(ec12_atheist)` and failures (non-athiest) are `r nrow(ec12)-nrow(ec12_atheist)` for the Ecuadorian respondents.  There are not enough successes to meet the minimum requirements (10 successes) so a margin of error estimate would not be appropriate for these results.
...
The number of successes (atheist) are `r nrow(sp05_atheist)` and failures (non-athiest) are `r nrow(sp05)-nrow(sp05_atheist)` for the 2005 data.  The number of successes (atheist) are `r nrow(sp12_atheist)` and failures (non-athiest) are `r nrow(sp12)-nrow(sp12_atheist)` for the 2012 data.  The minimum number of successes and failures have been met.
...
us05_atheist <- subset(atheism, nationality == "United States" & year == "2005" & response=="atheist")
us12 <- subset(atheism, nationality == "United States" & year == "2012")
us12_atheist <- subset(atheism, nationality == "United States" & year == "2012" & response=="atheist")
```

The number of successes (atheist) are `r nrow(us05_atheist)` and failures (non-athiest) are `r nrow(us05)-nrow(us05_atheist)` for the 2005 data.  The number of successes (atheist) are `r nrow(us12_atheist)` and failures (non-athiest) are `r nrow(us12)-nrow(us12_atheist)` for the 2012 data.  The minimum number of successes and failures have been met.
