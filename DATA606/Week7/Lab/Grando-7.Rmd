...
ggplot(residuals, aes(y=residuals, x=at_bats))+geom_point()+geom_hline(yintercept = 0)
```
...
Since the intercept is `r round(m1_int, digits = 2)` and the slopw is `r round(m1_slope, digits = 2)`, the linear regression line would predict `r round(m1_predict, digits = 2)` runs from 5.578 at bats.  The closest data point to 5,578 at bats is 5579 and returns `r m1_data`.  Therefore the prediction is an overstimate by `r round(m1_predict - m1_data, digits = 2)`.
...
ggplot(residuals, aes(y=residuals, x=bat_avg))+geom_point()+geom_hline(yintercept = 0)
```
...
ggplot(residuals, aes(y=residuals, x=new_onbase))+geom_point()+geom_hline(yintercept = 0)
...
ggplot(residuals, aes(y=residuals, x=new_slug))+geom_point()+geom_hline(yintercept = 0)
...
ggplot(residuals, aes(y=residuals, x=new_obs))+geom_point()+geom_hline(yintercept = 0)
```
